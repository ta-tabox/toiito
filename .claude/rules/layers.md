...
| 環境変数（認証） | `web/src/lib/auth/index.ts`（写像は `auth/config.ts` の `readAuthConfig`） | 組み立て済みの Better Auth |
...
- `process.env` を読むのは `lib/config.ts`・`lib/ai/providers.ts`・`lib/auth/index.ts`・`app/login/page.tsx` と、別プロセスで走る `scripts/`・`e2e/setup/` だけにする。
...
|---|---|---|---|
| ドメイン型 | `lib/types.ts` | `lib/question.ts`（型だけ） | 型だけ |
