...
この表が禁止語の正で、直下の `.coding-standards-vocab-banned` はここを機械が読める形へ写したもの（表を直したらこのファイルも直し、両者の一致はテストが見る）。
...
このリポジトリだけの禁止語は直下の `.coding-standards-vocab-deny` へ、この領域で比喩でない語は `.coding-standards-vocab-allow` へ、どちらも1行1語で足す（表と `.coding-standards-vocab-banned` は変えない）。
