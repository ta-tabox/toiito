...
`src/lib/ai/` を作り、次の二枚へ切り直す。

- `ai/index.ts` — プロバイダ非依存の呼び出し規約（`PersonaCall` / `QuestionRef` / `callPersona`）
- `ai/anthropic.ts` — Anthropic 固有の一切（`Effort` の値域・設定型・env からの読み・API 呼び出し）
...
- **プロンプトの組み立て（transcript の畳み方）を二つ目もそのまま使えると分かったとき**。
  `anthropic.ts` から `index.ts` へ引き上げる
- **深さを UI から触らせるとき**。
  独自語彙への正規化を再検討する
