# 0033. ログインを入れ、固定のユーザーを「Google を経ないサインイン」へ置き換える

- **状態**: supersede 済み（→ 0051）
- **決定日**: 2026-09-09
...
|---|---|---|
| 固定のユーザーのまま完了条件を満たせるか | 0031 の決定 5 は、環境変数が名指しする一人を常にサインイン済みにする。#68 の完了条件は、未サインインの状態と二人分のユーザーを E2E で見ることを要求する | E2E が完了条件の 2 項目を検査できない |
...
0031 の決定 5 が本番で固定のユーザーを許した根拠（外周は Basic 認証が持つ）も、ここで消える。
