...
- **Basic 認証を外す順序が、この回の作業と噛み合わない。**
  0031 の決定 5 は「ログインを入れる → 本番で確かめる → Basic 認証を外す」の三段を運用の規律として持つと決めているが、#68 の本文は `src/lib/basic-auth.ts` の削除をこの回に含めている。
  コードが消えれば、途中で止まる形そのものが無くなる
...
8. **Basic 認証の撤去とログインの投入を、1 回のデプロイで行う。**
   0031 の決定 5 が書いた三段（入れる → 確かめる → 外す）の途中で止まる形は取れない。
...
**0031 の決定 5 が書いた三段は、Basic 認証の判定がアプリのコードに在ることを前提にしている。**
`src/lib/basic-auth.ts` を残したまま #68 をデプロイすれば、確かめてから環境変数を消すという二段構えが取れる。
だが #68 の本文は `basic-auth.ts` の削除をこの回の作業に含めており、0013 の覆る条件も「#68 が入ったら外す」と書いている。
コードを残す判断はその両方に逆らうので、取らない。
...
戻すのは Vercel の Instant Rollback で、ルーティング層の切り替えなので秒で終わる（`DEPLOY.md`「切り戻し」）。

**`basic-auth.ts` を一回だけ残す案を採らなかった条件**: 残した回と外す回で本番が二度動くことになり、二度目の作業を誰も催促しない。
0013 が学んだのは、掛けたつもりの制限が差分に残らない場所で放置されることだった。
外し忘れた Basic 認証は害にはならないが、同じ形の放置を作る手順そのものを繰り返さない。
...
- **0031 の決定 5 が書いた三段が、決定 8 で 2 段になる**。
  デプロイして確かめる、までは同じで、三段目（Basic 認証を外す）は一段目と同時に起きる。
  `DEPLOY.md`「ログイン」がその手順を持つ
