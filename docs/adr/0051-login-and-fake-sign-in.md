# 0051. ログインを入れ、固定のユーザーを「Google を経ないサインイン」へ置き換える（0033 の改訂）

- **状態**: 採用
- **決定日**: 2026-09-15
- **関係する ADR**: 0013, 0018, 0022, 0029, 0030, 0031, 0033
...
0033 の決定 3 は本番での拒否を変数ごとの設定の読み取りに置いたので、同じ拒否を `TOIITO_FAKE_AI` の読み取りにも置くと、失敗する時点はそれぞれの読み取りが評価される時点で決まる。
認証の設定は最初のリクエストで、AI の設定はモジュールの評価時に読まれるので、同じ種類の設定の誤りが別々の時点と形で失敗する。

本 ADR は 0033 の決定 3 とその理由・帰結・覆る条件を改め、残りは 0033 から複写する。
...
| 採らなかった案 | 採らなかった条件 |
|---|---|
| フェイク専用の許可リストを持つ | 照合が二箇所になり、0022 の決定 9 が経路を一本に絞った理由（判定を差し替える場所が一つで済む）が消える |
...
| 各変数を読む関数が本番を判定する（0033 の決定 3） | 失敗する時点が読む関数の評価される時点で決まり、認証は最初のリクエスト、AI はモジュールの評価時とばらつく。変数を足すたびに同じ判定を書き写すことになる |
...
| 0033 | 本 ADR が supersede する。決定 3 とその理由・帰結・覆る条件だけが変わる |
| 0036 | 決定の節は `TOIITO_FAKE_LOGIN` の本番での扱いを「設定の読み取りが throw する（0033 の決定 1・3）」と書く。本番で拒否する場所は、本 ADR の決定 3 と読む |
