# セッションの守りを既定に委ねず、明示と実際の応答で持つ

- **状態**: 採用
- **決定日**: 2026-09-01
- **関係する ADR**: 20260829-production-basic-auth, 20260829-preview-neon-branch, 20260830-invite-only-multi-user, 20260830-auth-better-auth

## 文脈

20260830-auth-better-auth が決めたのは**認証の方式**であって、**セッションの守り方**ではない。
リポジトリを検索しても `CSRF` / `XSS` / `SameSite` / `HttpOnly` / セッション固定に触れた記述は一つも無かった（#149（セッションと認証まわりの守り方を決める）が確認）。

パスワードを持たない決定（20260830-auth-better-auth 決定 2）と OAuth のトークンを保存しない決定（同 決定 8）で、漏れたときに他サービスまで巻き添えにする資産は消えている。
...
版は 20260830-auth-better-auth が固定した 1.7.2 のままである（2026-09-01 に npm の `latest` で確認。2026-08-26 公開・MIT）。
...
10. **Preview の露出を引き受ける。**
    20260830-auth-better-auth 決定 7 のフェイク認証だけにし、Basic 認証は #68 で予定どおり外す。
...
一度目は 20260829-production-basic-auth（2026-08-29）である。
...
**20260830-auth-better-auth 決定 3 が Google OAuth 一本と決めた以上、窓を縮めるレバーは `expiresIn` だけである。**
...
20260830-invite-only-multi-user が anonymous を採らなかった条件（招待は宛先を要求し、anonymous は識別子を持たない）に、これで理由が一本増える——Better Auth の `anonymous` プラグインは匿名セッションをログイン後の利用者へ引き継ぐことが役目なので、決定 5 と正面から噛む。

### 決定 6 — `cookieCache` を有効にしない

根拠は**要件**である。
20260830-auth-better-auth 決定 4 が JWT を採らなかった条件は「発行済みを取り消せないので、#69（管理機能）の停止が次の期限切れまで効かない」だった。
...
守りの設定が壊れたときに素通しへ倒れる形は、20260829-production-basic-auth が Edge の `process.env` で一度踏んでいる。
...
1. この器の DB は Neon 一本でリードレプリカを持たない（20260829-neon-outside-vercel-marketplace）ので、解こうとしている問題が存在しない
...
**そして許可リストそのものが足場である。**
20260830-invite-only-multi-user 決定 3 は構成が固まるまでの持ち方として環境変数を選んだもので、本格運用ではアカウント登録制へ移り、明示的な許可リストは廃止する。
...
届いた先にあるのは、`pnpm seed` の内容と Preview ブランチで作った実験データだけである（20260829-preview-neon-branch が `Branch schema only` で切ると決めている）。
書き込みもできるが、`TOIITO_FAKE_AI=1` なので AI の費用は走らない（`DEPLOY.md`「Preview」）。
**20260830-auth-better-auth 決定 7 はこれを引き受けるとは書いていなかった**ので、ここで書く。
...
加えて 20260829-production-basic-auth の覆る条件「#68 が入ったら外す」に、Preview だけの例外を作ることになる。
**採る条件**: Preview に本番のデータが載る形へ変わったとき。

**Preview で本物のログイン画面を出す案を採らなかった条件**: 20260830-auth-better-auth が既に落としており、却下の条件が今も生きている。
...
20260829-production-basic-auth が学んだのは、設定でなく約束に頼った状態が「掛けたつもり」を作ることだった。
単独では採らない。
...
- **#111 は、この ADR の決定 10 で閉じる。**
  #111 の三候補（引き受ける / 明示のオプトアウト / cookie 形へ差し替え）はいずれも Basic 認証の話で、#68 が入れば 20260829-production-basic-auth の覆る条件が発火して対象ごと消える。
  つまり #111 は #68 の到着で自動的に閉じる issue だった
- **20260829-production-basic-auth は覆らない。**
  覆る条件の一つ目（#68 が入ったら外す）が変わっておらず、決定 10 はその予定を Preview にも適用すると確認しただけである
- **20260830-auth-better-auth は supersede しない。**
  決定 7（Preview と E2E はフェイク認証）は動いておらず、書かれていなかった引き受けの条件をこの ADR が足している
- **20260830-invite-only-multi-user 決定 3（許可リストは環境変数）に、照合の時点と有効期限が付く。**
  20260830-invite-only-multi-user は持ち方だけを決めていて、いつ照合するかを決めていなかった
- **#68 の「作るもの」が増える。**
...
- **Preview に本番のデータが載ったら。**
  ...
  20260829-preview-neon-branch の `Branch schema only` を変える判断は、この決定を同時に動かす
...
- **`React.cache()` の重複除去を入れた上でなお DB 往復が実測で効くと分かったら。**
  ...
  20260830-ownership-granularity が同じ規律を書いているとおり、**段数や勘で遅いと決めない**
