# Preview に Neon のブランチを当てた専用の DB を持たせる

- **状態**: 採用
- **決定日**: 2026-08-29
- **関係する ADR**: 20260829-production-basic-auth
...
**ブランチは schema only で切り、`pnpm seed` の内容を入れる。**
...
20260829-production-basic-auth の「設定が無いときに素通しへ倒さない」がそのまま効いており、**掛け忘れた Preview が黙って開くことは構造上できない**。

**親のデータをコピーする既定を採らなかった条件**: Neon のブランチは既定で親の HEAD をコピーするので、切っただけでは本番の問いがそのまま Preview に入る。
書き込みは分離されるので本番は汚れないが、自分の問いの置き場が黙って二つになる。
画面を確かめる用途には seed の方が向いてもいる——何が入っているかが決まっているので、見えているものが変更のせいかデータのせいかを切り分けられる。
**切った後に消す案を採らなかった条件**: 消し忘れと接続先の取り違えが挟まる。
schema only なら本番のデータが Preview へ入る瞬間そのものが無いので、条件を手順でなく構造で満たせる。
...
- **schema only は `_prisma_migrations` も空にする**（2026-08-29 に実測）。
  テーブルは在るのに Prisma からは migration が一つも当たっていないと見えるので、切った直後に `migrate resolve --applied` で辻褄を合わせる手当てが要る（綴りは `DEPLOY.md`「Preview」）

## 覆る条件

- **#68（ログイン（Google OAuth）とリソースの所有権）が入ったら、Basic 認証の 2 本は消える**（20260829-production-basic-auth と同じ）。
