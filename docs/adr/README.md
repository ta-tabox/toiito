...
| [0028](0028-ownership-before-auth.md) | 所有者を認証より先にデータ層へ入れ、Better Auth の四表はモデル名も列名も生成されたまま同居させる | 2026-09-02 | 採用 |
...
- **ステップ一覧 S0〜S6 と、その進め方の横断規約**（2026-07-13 起草の `PLAN.md` 由来）。
  順序と契約であって決定ではないので、`ROADMAP.md`（順序）・`.claude/rules/`（規約）・`HARNESS.md`（検証）が既に持つ
