...
- **Prisma を repo 層の外へ出さない**。
  ...
  唯一の例外は `db.ts` の `authDatabaseClient` で、Better Auth のアダプタがクライアントそのものを要求するために開けてある（呼んでよいのは `lib/auth.ts` だけ）
...
**現在のユーザーを返す口は `lib/current-user.ts` の `getCurrentUser` 一つ**で、RSC と Server Action は `requireCurrentUser` を通ってから repo 関数を呼ぶ。
...
Google を経ないサインイン（`TOIITO_FAKE_LOGIN=1`）は Preview と E2E だけが使い、本番では起動時に拒否される（`adr/0032-login-and-fake-sign-in.md`）。
...
一本にまとめない理由は実損から来ている。
手元で先行して運用していた問いの記録で、2026-07-01 に捕捉された問いが同音近接（「癒える」→「言える」）で誤記され、**誤った語のために自動収集された材料を積んだまま18日間もっともらしく滞留した**。
...
旧3値の `closed` は**三つの別物**（書き出した / 解決して閉じた / 捨てた）を潰しており、`exported` / `resolved` / `discarded` へ割れている。

`permanent` は VISION の開いた問い「発酵した状態の判定・表現」への部分回答。
収束だけを終点に置く設計は、収束させて畳むこと自体が損失になる種類の問いを潰す。
発酵の終点は一つではなく、**結晶（exported）と持続（holding / permanent）が並立する**。
値域は二箇所で表明する。
...
└── web/               Next.js アプリ本体
    ...
    │   ├── lib/           db.ts（Prisma repo 層）・auth.ts（Better Auth）・current-user.ts（現在のユーザー）・ai/（AI 呼び出し）・personas.ts・anchors.ts
