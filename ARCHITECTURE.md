...
- **「答えを与えない」の実装可能な形**（2026-07-19 追加）。
  ...
  実地の観測では、外部知識を大量に供給しても問いは潰れなかった——供給されたのが**結論ではなく係争中の材料**だったから（矛盾する研究を矛盾したまま渡し、解消は人間がした）。
...
- **具体／抽象の適切さをどう決めるか**（2026-07-20 追加）。
  ...
  注意: 死別支援の「粒度の決定権は傷を負った側にある」は**ケアの場の規律であってここへ移植できない**——思考の場で決定権を渡し切ると相手は自分の重心から動かない。
