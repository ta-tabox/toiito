...
| S5 | AI 応答のストリーミング化 | Claude | **条件付き**: S0 実機確認で体感が重い場合のみ |
...
**帰結**: 実対話 OK・体感は遅くない → **S5 は見送り**（常用で覆れば再開）。
派生要件: 発話送信中のローディング表示 → **S2 に追加**。
...
## S5 AI 応答のストリーミング化（条件付き）

**着手条件**: S0 の実機確認で「二体分を同期で待つのが体感的に重い」と
人間が判定した場合のみ。軽ければこのステップは**やらずに閉じる**。
