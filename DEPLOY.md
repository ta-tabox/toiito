...
幅の主因は**出力の長さ**だと見ている。
一往復は `max_tokens: 1024` を二回逐次で待つ形なので、二体の応答が長い回は素直に伸びる。
27 秒の回は 5 分以上空けた後だったので Neon の復帰も乗っているが、接続一回にその差は乗りにくい。

**切り分けるなら、5 分以上空けてから `/` を開く**。
問い一覧は DB を読むだけで AI を呼ばないので、そこで測れるのは Vercel の関数のコールドスタートと Neon の復帰だけになる。
