...
#
# 語の一覧の正本は `.claude/rules/writing.md`「語彙と読み手」節の表で、
# `BANNED_WORDS` はそれを機械が読める形へ写したもの(表を直したらここも直す)。
...
# 単漢字の語(器・口・印)が無関係な複合語に埋もれる誤検出は、代表例を STRIP_COMPOUNDS が
# 判定用のコピーからだけ取り除く(報告する本文は取り除く前の原文のまま)。
#
# 使い方:
