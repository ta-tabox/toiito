...
- **MVP の縦一本の残り**: #4（メモ UI）→ #6（E2E）→ #7（MVP 到達判定）。
  #1 / #2 / #3 / #5 / #11 は完了済み。
  #5 の `/memos` は既に `#msg-<id>` へリンクしているが着地先がまだ無く、それを埋めるのが #4
- **別トラックで開発ハーネスが開いている**（ROADMAP の依存グラフには載っていない）。
  #16（`pnpm check` を GitHub Actions で回す）は完了。
  残りは #33（テスト隔離のケース単位化）・#30（`next dev` の AGENTS.md 書き換え停止）・#18（Actions 上の @claude に check を通させる）・#34（ブランチ保護）。
  #19（Actions 経由の Claude の名義）は人間の判断待ち、#20（公開時のトリガ制限）は公開を決めるまで着手しない栞
- **並行の可否を検証した**（2026-08-19）。
  #33 / #4 / #30 / #18 / #34 は触るファイルが互いに素なので、並行に走らせられる。
  順序の縛りは **#33 → #4** の一本だけ——#4 が足す DB を触るテストは、隔離の導入前だと自分の行を `randomUUID()` で切り出す細工を書くことになり、#33 が後から入ると書き直しになる。
  #30 は案の採否、#34 は GitHub の設定が人間の担当
