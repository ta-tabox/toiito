 ...
 *
 * 一度に描くのはセッション一つ。
 * 既定は最新で、`?s=<session_id>` が指すセッションがあればそのセッションを描く。
 * 過去のセッションは読み取り専用にする。
 * 再訪は前の続きではなく「また話す」ことなので、足したい発話は最新のセッションへ行く（docs/ARCHITECTURE.md「再訪と、過去セッションの読み方」）。
 *
 * 発話の生成と永続化は Server Action の担当。
 * 本文の描画と選択からのメモ作成は `MessageBody` の担当。
 * `QuestionPage` は並べて描くところまで。
 *
 ...
 * `ai_a` と `ai_b` はどちらも左寄せで位置が同じなので、`ai_a` には暖色の背景を、`ai_b` には寒色の背景と枠線を当てて分ける。
