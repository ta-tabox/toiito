/**
 * メモの一覧と、メモから出所の発話への逆引き。
 *
 * 取得は `listMemosWithContext` 一本。
 * 行ごとに問いやセッションを取得し直さない。
 *
 * 各行を押すとメモ一件が `?memo=<id>` で拡大表示される。
 * 開いているメモを URL で持つのは、対話画面の下線から特定のメモを名指しで開く経路があるため。
 * client の state に持つと、対話画面から特定のメモを指す手段が無くなる。
 *
 * 出所の発話への逆引き（`/q/<question_id>?s=<session_id>#msg-<message_id>`）は拡大表示の中に置く。
 * セッションを名指しするのは、再訪で最新が入れ替わってもメモを付けた当時の発話へ着地させるため。
 * セッションを省くと問いの画面へは遷移するが、着地先が DOM に無く、何も起きずに終わる。
 * ジャンプはこの画面だけでは完結せず、三箇所が同じ書式を共有して初めて成立する。
 * URL を書くのが `memos/page.tsx`、飛び先の `id="msg-…"` を発話へ付けるのが `q/[id]/page.tsx`、飛んだ先でその発話を強調するのが `landing-mark.tsx`。
 * 書式を変えるときは三箇所とも直す。
 */
