 ...
 * 開いているかどうかの正は URL のクエリ（`?memo=<id>`）で、`MemoDialog` は state を持たない（理由は `memos/page.tsx`）。
 *
 * `<dialog>` を使うのは Esc・背景の不活性化・フォーカスの閉じ込めを自前で書かないため。
 * 閉じる手段は Esc と閉じるボタンの二つで、背景を押して閉じる形は入れない。
 * `<dialog>` の背景は擬似要素で単独では押せず、dialog 自身の click を見る形になるので、キーボードから届かない操作が一つ増える。
 *
 * `backdrop` だけは色のトークンを使わない。
 * 背後を暗くするためのもので明暗の二系統を持たず、ダークテーマのトークンを当てると明るい面が被さる。
 */
