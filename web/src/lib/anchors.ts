// 純関数のみ・DB / next / DOM 非依存。メッセージ本文とメモのアンカー区間から、表示用の分割・
// オフセット換算・引用生成を行う（対話画面・逆引き画面の両方が使う）。
//
 ...
 * 切断点はメモの境界そのものなので、各セグメントは「特定のメモ集合を完全に内包する／しない」の
 * どちらかに定まり、メモ区間がセグメント途中で切れることはない。
 */
 ...
 * 丸め方向は常に手前（index - 1）——start / end どちらの用途で呼ばれても「元の index を超えない」を
 * 保証すれば、本文範囲をはみ出す事故もペア片割れの分断も起きない。
 */
