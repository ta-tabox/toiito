...
/**
 * 本文を切り分けた一区間。「掛かっているメモの組み合わせが変わらない最大の連続範囲」で、UI はこれを 1 単位として下線を描く。
 *
 ...
 *
 * メモは本文の一部に付く下線で、複数のメモが同じ範囲に重なりうる。UI は重なりの有無が切り替わるたびに描き分けたいので、切り替わる位置＝全メモの端点で本文を割る。
...
export function segmentBody(body: string, memos: AnchorRange[]): Segment[] {
  ...
  // 隣り合う切断点の間が 1 セグメント。切断点はメモの端点そのものなので、メモ区間がセグメントの途中で切れることはない。
 ...
 *
 * コードポイント境界では足りない。異体字セレクタ（神︀ = U+795E + U+FE00）や結合文字はサロゲートペアを含まないので素通ししてしまい、肌色修飾や ZWJ 連結の絵文字（👨‍👩‍👧 = 8 code unit）は内部の境界で割れる。いずれも分断すると字体や絵柄が変わる。
 *
...
/**
 * メモの引用を作る。アンカー区間（メモの anchor_start / anchor_end）の前後へ margin 文字ずつ広げた範囲を返し、本文の端と書記素境界で止める。逆引き一覧（#5）が使う。
