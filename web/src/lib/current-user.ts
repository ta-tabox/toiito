...
export function readSingleUserEmail(env: SingleUserEnv): string {
  ...
  if (!email) {
    throw new Error(
      "TOIITO_SINGLE_USER_EMAIL が設定されていない。ログインが入るまで、現在の利用者はこの環境変数が名指しする（web/README.md「環境変数」）",
    );
  }
...
export const getCurrentUser = cache(async (): Promise<User> => {
  ...
  if (!user) {
    throw new Error(
      `TOIITO_SINGLE_USER_EMAIL が名指しする利用者が DB に居ない: ${email}。手元と Preview は pnpm seed、本番は DEPLOY.md「唯一の利用者」の手順で入れる`,
    );
  }
