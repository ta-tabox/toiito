 ...
 *
 * 判定の単体テスト（`tests/basic-auth.test.ts`）と `proxy` の戻り値のテスト（`tests/proxy.test.ts`）は、関数が正しい値を返すことしか言えない。
 * サーバーが本当に 401 を返すか、制限がすべての経路に掛かっているかは、ブラウザから叩くこの spec でしか見えない。
 *
 * 資格情報を持つサーバーは別に立てる（`playwright.config.ts` の basic-auth プロジェクト）。
 * 他の spec が使うサーバーは資格情報を持たないので、認証を要求しないまま変わらない。
 *
 * 叩くのは存在しない経路にする。
 * データベースへ問い合わせずに済むので他の spec が作り直している最中でも結果が揺れず、アプリのルートでない場所が 401 になることは制限が routing より前に掛かっている証拠にもなる。
 */
