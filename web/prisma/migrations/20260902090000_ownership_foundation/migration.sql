...
-- 出力に無い文は「唯一のユーザー」と「DataMigration」の二つで、残りは出力のままである。
--
-- 既存の問いは消さずに、唯一のユーザーへ寄せる（docs/adr/0031-ownership-before-auth.md 決定 6）。
...
--
-- 受け皿の email は placeholder で、本番では人間が自分のものへ差し替える（docs/DEPLOY.md「唯一のユーザーの行を入れる」）。
...
-- InsertData: 持ち主のいない問いがあるときだけ、受け皿のユーザーを一人作る
-- 問いが一件も無い DB（テスト・E2E・立ち上げ直後）では、この文も次の文も何もしない。
INSERT INTO "user" ("id", "name", "email", "emailVerified", "createdAt", "updatedAt")
SELECT gen_random_uuid()::text, '唯一のユーザー', 'owner@toiito.invalid', false, now(), now()
